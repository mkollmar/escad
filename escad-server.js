// Copyright (C) 2011, 2012, 2013, 2014, 2019, 2020, 2021, 2022 Markus Kollmar (email: markuskollmar@onlinehome.de)
//
// This server implements a universal network connection-manager for escad.
// It provides a REST-interface for escad, handles connections to graph-databases, fetch http-sites or other network connection which may occur in future.
//
// For this we communicate with at least two sides:
// [escad] <-> [rest-server.js] <-> [html-browser + escad-web-interface]
//                              <-> [arangodb-server (optional)]

// ******** CONFIG START
const port2escad = 3000;
const host2escad = '127.0.0.1';
const port2browser = 4000;
const port2arangodb = 8529;
// ******** CONFIG END
...
// for arangodb (graph-database) where you can use gui to edit escad:
import { Database, aql } from "arangojs";
const db = new Database({ url: "http://localhost:8529" });
db.useDatabase("thesis");
