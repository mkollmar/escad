;;; graphical inspector with tk 8.4

(defpackage :escad-gui-tk
  (:use :common-lisp
	:ltk
	;"SB-IMPL"
	)
...
; inspired from: ginspect
(defun start-gui (inspected-value)
  "inspect the value with a graphical user interface"
  (let ((*debug-tk* nil) (*wish-args* '("-name" "ESCAD client (TK)")))
  (with-ltk ()
   (setf *debug-tk* t)
   (let* ((inspector (make-instance 'ginspector))
	  (f (make-instance 'frame))
	  (f_sym (make-instance 'labelframe :text "symbol"))
	  (f_rel (make-instance 'labelframe :text "relation"))
	  (lbl (make-instance 'label :text "Loaded"))
	  ...
	  (history nil)
	  (history-pos 0)
	  (new_sym (make-instance 'button :master f_sym :text "New"
				  :command (lambda ()
					     (with-widgets
						 (toplevel top-frame :title "Create new symbol"
							   (label lb1 :text "Symbol name (leave empty to auto-generate):" :pack '(:side :top))
							   (entry en1 :pack '(:side :top) :text "")
							   (label lb2 :text "Taxonomy:" :pack '(:side :top))
							   (entry en2 :pack '(:side :top) :text "")
							   (label lb3 :text "" :pack '(:side :bottom))
							   (frame fr1 :pack '(:side :bottom)
								  (button bt1 :text "OK" :pack '(:side :right)
									  :command (lambda () (setf (text lbl) "execute: ns") (setf (text lb3) (format nil "creating... ~a~%" (escad:ns en1)) )))
								  (button bt2 :text "CANCEL" :pack '(:side :left)
									  :command (lambda () (withdraw top-frame))))

							   ) ))))
	  (edit_sym (make-instance 'button :master f_sym :text "Edit"
			       :command (lambda () (setf ltk::*exit-mainloop* t))))
	  (rm_sym (make-instance 'button :master f_sym :text "Remove"
			       :command (lambda () (setf ltk::*exit-mainloop* t))))
	  (act_sym (make-instance 'button :master f_sym :text "Activate"
			       :command (lambda () (setf ltk::*exit-mainloop* t))))
	  (new_rel (make-instance 'button :master f_rel :text "New"
			       :command (lambda () (setf ltk::*exit-mainloop* t))))
	  (edit_rel (make-instance 'button :master f_rel :text "Edit"
			       :command (lambda () (setf ltk::*exit-mainloop* t))))
	  (quit (make-instance 'button :master f :text "Quit"
			       :command (lambda () (setf ltk::*exit-mainloop* t))))
	  (load_view (make-instance 'button :master f :text "Load view"
			       :command (lambda ()
					  (escad:lov (get-open-file :title "Select a view-file (.lisp) to load!")))))
	  (help (make-instance 'button :master f :text "help"
			       :command
			       (lambda ()
				 (setf (text (title inspector))
				       (format nil "Type (help)")))))
	  (view (make-instance 'button :master f :text "Make PDF"
			       :command 
			       (lambda ()
				 (escad:as "_view") ; generate new pdf
				 (setf (text (title inspector))
				       (format nil "Open/reload file ~a (Evince automatically reloads, Adobe requires Ctrl + R.)." (escad:gsa "_view" "filename_relative"))))))
     ...
     (pack quit :side :left)
     (pack back :side :left)
     (pack load_view :side :left)
     (pack new_sym :side :left)
     (pack edit_sym :side :left)
     (pack rm_sym :side :left)
     (pack act_sym :side :left)
     (pack new_rel :side :left)
     (pack edit_rel :side :left)
     (pack view :side :right)
     (pack help :side :right)
     (pack f :side :top :fill :x)
     (pack f_sym :side :top :fill :x)
     (pack f_rel :side :top :fill :x)
